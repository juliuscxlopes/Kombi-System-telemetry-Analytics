...
class Classificador {
  ...
  /**
   * Sistema de votação entre 4 dimensões independentes
  ...
  classificar(sensorName, deltaJanela, eta, deltaTicket) {
    ...
    // ── RETORNO AO NOMINAL: fecha direto, sem gradação ───────
    if (deltaJanela.valor !== undefined && deltaJanela.valor <= specFisica.NOMINAL_MIN) {
      logger.info(`✅ [CLASSIFICADOR:${sensorName}] Sensor abaixo do NOMINAL_MIN (${specFisica.NOMINAL_MIN}°C) — retorno operacional.`);
      return { nivel: 'TOLERAVEL', motivos: ['retorno_nominal'], predictive: null, votos: {} };
    }

    // ── VOTAÇÃO DAS 4 DIMENSÕES ───────────────────────────────
    const votos = {
      ...
      eta_alerta:   this._votarETA(eta.etaAlertaMinutos),
    ...
    if (nivel !== 'TOLERAVEL') {
      ...
      if (specPreditiva) {
        ...
        logger.warn(`⚠️ [CLASSIFICADOR:${sensorName}] Nível '${nivel}' sem configuração preditiva em metrics_specs.json.`);
      }
    }

    logger.info(`🩺 [CLASSIFICADOR:${sensorName}] Nível: ${nivel} | Votos: taxa=${votos.taxa} delta=${votos.delta_janela} proj=${votos.projecao} eta=${votos.eta_alerta} ticket=${votos.delta_ticket} | Predictive: ${predictive?.tipo ?? 'null'}`);
  ...
  _votarTaxa(valor, limites) {
    if (valor == null) return 'TOLERAVEL';
  ...
  _votarProjecao(projecao30s, limites) {
    if (projecao30s == null) return 'TOLERAVEL';
  ...
  /**
   * ETA nulo (taxa <= 0, sensor resfriando) = voto explícito de TOLERAVEL
   * ETA presente = urgência inversamente proporcional ao tempo restante
   */
  _votarETA(etaAlertaMinutos) {
    if (etaAlertaMinutos == null) return 'TOLERAVEL';
    if (etaAlertaMinutos <= 1)   return 'PREDICTIVE_4';
    if (etaAlertaMinutos <= 2)   return 'PREDICTIVE_3';
    if (etaAlertaMinutos <= 4)   return 'PREDICTIVE_2';
    if (etaAlertaMinutos <= 8)   return 'PREDICTIVE_1';
    return 'TOLERAVEL';
  }
  ...
  _resolverNivel(votos, deltaTicket) {
    ...
    // Contagem dos 4 votos de dimensão (ticket é double check, não vota aqui)
    ...
    contagem[votos.eta_alerta]   = (contagem[votos.eta_alerta]   ?? 0) + 1;

    // Nível base: mínimo 2 votos concordando no mesmo nível
    ...
    // ── DOUBLE CHECK: delta_ticket ────────────────────────────
    // CRITICO  → escala direto para P4 independente dos votos
    if (deltaTicket.estado === 'CRITICO' && HIERARQUIA[nivelBase] < HIERARQUIA['PREDICTIVE_4']) {
      return 'PREDICTIVE_4';
    }

    // PIORANDO → garante mínimo P2 (contenção não funcionou)
    if (deltaTicket.estado === 'PIORANDO' && HIERARQUIA[nivelBase] < 2) {
      return 'PREDICTIVE_2';
    }

    // SAUDAVEL → descida gradual: desce 1 nível por ciclo
    if (deltaTicket.estado === 'SAUDAVEL' && HIERARQUIA[nivelBase] > 0) {
      const nivelAbaixo = Object.keys(HIERARQUIA).find(k => HIERARQUIA[k] === HIERARQUIA[nivelBase] - 1);
      return nivelAbaixo ?? nivelBase;
    }
  ...
  _montarMotivos(votos, deltaJanela, eta, deltaTicket) {
    ...
    if (votos.delta_janela !== 'TOLERAVEL') motivos.push(`delta_janela: ${deltaJanela.delta}°C → ${votos.delta_janela}`);
    if (votos.projecao     !== 'TOLERAVEL') motivos.push(`projecao_30s: ${eta.projecoes?.em30s}°C → ${votos.projecao}`);
    if (votos.eta_alerta   !== 'TOLERAVEL') motivos.push(`eta_alerta: ${eta.etaAlertaMinutos}min → ${votos.eta_alerta}`);
